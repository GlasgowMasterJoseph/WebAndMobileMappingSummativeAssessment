...
const map = new mapboxgl.Map({
  ...
  center: [-81.719497122, 27.922829642], // Initial location set to Florida
  zoom: 5 // Initial zoom
});
...
const geocoder = new MapboxGeocoder({
  ...
  mapboxgl: mapboxgl,
  marker: false,
  placeholder: "Search places in Florida", // Placeholder text
...
map.on('click', async (event) => {
  ...
  try {
    ...
    // Check if the airport code is blank and set it to "Unavailable" (OpenAI ChatGPT, 2025) 
    if (!airportCode || airportCode.trim() === "") {
      airportCode = "Unavailable";
    }

    // Update weather information box
...
class PitchControl {
  ...
  _togglePitch() {
    ...
    if (currentPitch === 0) {
      this._map.setPitch(60); // Custom pitch set for 3D look
